...
namespace UtfN
{
	...
	{
		return Utf8StringToUtf16String<utf16_char_string>(
			utf8_iterator<inner_iterator>(std::begin(StringToConvert), std::end(StringToConvert)));
	}
		...
		string_type WStringToString(const wstring_type& WideString)
	{
		...
		if UTF_IF_CONSTEXPR (sizeof(char_type) == 0x2) // UTF-16
		{
			...
			return Utf16StringToUtf8String<type_to_use>(UtfImpl::Utils::ForceCastIfMissmatch<const type_to_use&, const wstring_type&>(WideString));
		}
		else // UTF-32
		{
			using type_to_use = typename std::conditional<sizeof(char_type) == 4, wstring_type, dummy_4byte_str>::type;
			return Utf32StringToUtf8String<type_to_use>(UtfImpl::Utils::ForceCastIfMissmatch<const type_to_use&, const wstring_type&>(WideString));
		}
	}
		...
		wstring_type StringToWString(const string_type& NarrowString)
	{
		...
		{
			using type_to_use = typename std::conditional<sizeof(char_type) == 2, wstring_type, dummy_2byte_str>::type;
			return Utf8StringToUtf16String<type_to_use>(NarrowString);
		}
		else // UTF-32
		{
			using type_to_use = typename std::conditional<sizeof(char_type) == 4, wstring_type, dummy_4byte_str>::type;
			return Utf8StringToUtf32String<type_to_use>(NarrowString);
		}
	}
